...
const SURFACE_FILL = [244, 248, 251];
const SURFACE_BORDER = [222, 232, 238];
const TEXT_PRIMARY = [28, 42, 54];
const TEXT_MUTED = [90, 112, 126];
const BRAND_DARK = [35, 74, 101];
const POSITIVE = [22, 128, 110];
const NEGATIVE = [189, 84, 58];

const assetAccentBySymbol = {
  BTC: [42, 120, 132],
  ETH: [87, 168, 155],
  SOL: [150, 195, 155],
  XRP: [216, 179, 122],
...
function getReportMonthLabel(monthlyPerformance) {
...
function drawPageFooter(doc, pageNumber) {
  ...
  doc.text(`Page ${pageNumber}`, PAGE_WIDTH - PAGE_MARGIN, PAGE_HEIGHT - 9, { align: 'right' });
}

function drawSectionCard(doc, x, y, width, height) {
...
function drawSectionTitle(doc, eyebrow, title, subtitle, x, y) {
  doc.setFontSize(8);
  doc.setTextColor(...TEXT_MUTED);
...
function addCoverHeader(doc, portfolio, summary, reportMonthLabel) {
...
function addLivePricesSnapshot(doc, livePrices, pricesLastUpdatedAt, startY) {
  ...
  livePrices.slice(0, 6).forEach((price, index) => {
...
function addAllocationSection(doc, holdings, startY) {
...
function drawDonutSegment(doc, centerX, centerY, outerRadius, innerRadius, startAngle, sweepAngle, color) {
  const points = [];
  ...
  for (let index = 0; index <= steps; index += 1) {
    ...
    points.push([centerX + outerRadius * Math.cos(radians), centerY + outerRadius * Math.sin(radians)]);
  }

  for (let index = steps; index >= 0; index -= 1) {
    const angle = startAngle + (sweepAngle * index) / steps;
    const radians = (angle * Math.PI) / 180;
    points.push([centerX + innerRadius * Math.cos(radians), centerY + innerRadius * Math.sin(radians)]);
  }
...
function addPerformanceSection(doc, monthlyPerformance, startY) {
  ...
  if (monthlyPerformance.length > 0) {
    ...
    for (let tickIndex = 0; tickIndex <= tickCount; tickIndex += 1) {
      ...
      doc.setFontSize(8);
      doc.setTextColor(...TEXT_MUTED);
      doc.text(formatRoundedCurrency(tickValue), plotLeft - 8, y + 1, { align: 'right' });
    }
    ...
    points.forEach((point) => {
      ...
      doc.setFontSize(7.5);
      doc.setTextColor(...TEXT_MUTED);
      doc.text(point.label, point.x, plotBottom + 5, { align: 'center' });
    });
  }
...
function addHoldingsTable(doc, holdings, startY) {
  ...
  autoTable(doc, {
    ...
    theme: 'plain',
    headStyles: { fillColor: SURFACE_FILL, textColor: TEXT_PRIMARY, fontStyle: 'bold' },
    bodyStyles: { textColor: TEXT_PRIMARY, cellPadding: 3, valign: 'middle' },
    ...
    styles: { fontSize: 8.6, overflow: 'linebreak' },
    pageBreak: 'avoid',
    rowPageBreak: 'avoid',
...
export function buildInvestmentReportFilename(portfolioName, reportMonthLabel) {
...
export function createInvestmentReportPdfDocument(data) {
  ...
  addHoldingsTable(doc, data.holdings, performanceEndY + 18);
